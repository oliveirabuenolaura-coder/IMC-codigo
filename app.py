...
if st.button("Calcular IMC"):
    if peso > 0 and altura > 0:
        ...
        if imc < 18.5:
            ...
            st.write("Classificação: Peso ideal (parabéns!)")
        elif 25 <= imc < 30:
            st.write("Classificação: Levemente acima do peso")
        elif 30 <= imc < 35:
            st.write("Classificação: Obesidade Grau I")
        elif 35 <= imc < 40:
            st.write("Classificação: Obesidade Grau II (severa)")
        else:
            st.write("Classificação: Obesidade Grau III (mórbida)")
    else:
        st.write("Por favor, insira valores válidos para peso e altura.")
